...
public:
    ...
    void ProcessKeyboard(Camera_Movement direction, float deltaTime)
    {
        ...
        if (direction == RIGHT)
            Position += Right * velocity;
        if (direction == UP)
            Position += WorldUp * velocity; // Движение вверх
        if (direction == DOWN)
            Position -= WorldUp * velocity; // Движение вниз
    }
    ...
    glm::mat4 calculate_lookAt_matrix(glm::vec3 position, glm::vec3 target, glm::vec3 worldUp)
    {
        ...
        // 2. Получаем положительный вектор правой оси
        glm::vec3 xaxis = glm::normalize(glm::cross(glm::normalize(worldUp), zaxis));
        // 3. Вычисляем вектор камеры вверх
        ...
        rotation[0][1] = yaxis.x; // второй столбец, первый ряд
        rotation[1][1] = yaxis.y;
        rotation[2][1] = yaxis.z;
        rotation[0][2] = zaxis.x; // третий столбец, первый ряд
        rotation[1][2] = zaxis.y;
        rotation[2][2] = zaxis.z;

        // Создаем матрицу трансляции
        glm::mat4 translation = glm::mat4(1.0f); // единичная матрица по умолчанию
        translation[3][0] = -position.x; // четвёртый столбец, первый ряд
        translation[3][1] = -position.y; // четвёртый столбец, второй ряд
        translation[3][2] = -position.z; // четвёртый столбец, третий ряд

        // Возвращаем результат перемножения матриц
